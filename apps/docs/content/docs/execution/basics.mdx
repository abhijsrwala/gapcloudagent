...
description: Understanding the fundamental execution flow in GapCloud Agent
---
...
When you run a workflow in GapCloud Agent, the execution engine follows a systematic process to ensure blocks are executed in the correct order and data flows properly between them.
...
Run workflows on-demand through the GapCloud Agent interface by clicking the "Run" button. This is perfect for:
